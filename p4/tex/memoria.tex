...
Veamos ahora el algoritmo implementado en el lenguaje \verb|C++|. En primer lugar, hemos separado la implementación del \textit{criterio de factibilidad}, para que se aprecie con claridad:

	\lstinputlisting[language=C++, linerange={19-24}, caption=Criterio de Factibilidad]{./../src/Algoritmo.cpp}
\vspace{1em}

Ahora pasemos a la función principal. El algoritmo comienza comprobado si está en el último paso del algoritmo, de ser así habrá encontrado solución y terminará.

El algoritmo se repetirá con cada una de las casillas a las que puede moverse el caballo desde la posición en la que se encuentra. Dichas posiciones se encuentran en un vector que se irán sumando a la posición actual para ir calculando las nuevas. Para cada posición se comprobará si es factible la posición con la función anteriormente mostrada.

Si no encontramos una solución por algún camino se podrá un $0$ en las casillas por las que se haya pasado, en caso contrario, es decir, que se encuentre solución se irá devolviendo \verb|true|.
...
\lstinputlisting[language=C++, linerange={26-49}, caption=Algoritmo BackTraking]{./../src/Algoritmo.cpp}

Por último veamos el \verb|main| diferenciamos varios casos:
\begin{itemize}
	\item Si no se pasa como parámetro la posición inicial generaremos una posición aleatoria hasta que encontremos una solución al problema
	\item El tamaño del tablero si es menor que 5x5 nunca se obtendrá una solución, por tanto solo se ejecutará el algoritmo si es mayor o igual a 5. Si el tamaño es uno la solución será la única casilla
\end{itemize}
...
%%TODO: completar. Comentar que siempre encuentra una solución, pero en tiempos inviables).
