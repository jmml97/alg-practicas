...
/**
 * @brief Mezclar k vectores mediante el algoritmo Divide y Vencer�s
 * @param T matriz con los vectores
 * @param n tama�o de los vectores
 * @param p posici�n inicial del vector en la matriz T
 * @param f posici�n final del vector en la matriz T
 */
int* mezclaDV(int** T, int n, int p, int f) {
  //Caso general
  int k = (f - p + 1) / n; //N�mero de vectores
  if(k > 2) {
    //Dividivimos el n�mero de vectores que tomamos
    int p1 =  p;
    int f1 = (f - p) / 2;
